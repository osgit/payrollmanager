...
public class MainView extends JFrameView implements TableModelListener {
    private final JTabbedPane m_tabs; //holds the tabs for the user/admin view
    private final Handler m_handler = new Handler(); //event listener for class

    public MainView(MainModel model, MainController controller) {
        super(model, controller);
        m_tabs = new JTabbedPane();
        m_tabs.add(new JLabel(createImageIcon("payroll-manager.png",
                "Application's main image")));
        this.setTitle("Payroll Manager -- Tech Startup");
    ...
    public void setView(UserClass userClass) throws PayrollException {
        m_tabs.removeAll(); //remove the payroll-manager image
    ...
    private void createNodes(DefaultMutableTreeNode root) {
         ...
         * For each month create a DefaultMutableTreeNode and root.add() it then
         * for each
         * pay period within that month create another DefaultMutableTreeNode
         * and then
         * add that to the month node.
         *
         * Alternatively we could extend TreeModel similarly to the way
         * EmployeeTableModel extends
         * AbstractTableModel. I haven't looked into it too deeply but it might
         * facilitate handling
    ...
    public static void main(String[] args) {
        try {
            ...
            MainController.handlePayrollException(pe);
        }
    }
}
